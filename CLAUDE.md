...
- **Phase 6** — Inventory + recipes + tile entities
  ...
  - **6c** — Tile entities + crafting + chests/furnaces. **Shipped.**
    - **Wire format**: `PlayerInteractBlock` (0x46) friend→server intent, `OpenWindow` (0x52) server→friend, `CloseWindow` (0x53) bidirectional, `WindowClick` (0x54) friend→server slot click, `TileEntityData` (0x60) server→friend slot snapshot. New `WindowKind` constants (Chest=1, Furnace=2, CraftingTable=3).
    - **Server**: `ServerClient.OpenWindows` dict + `OpenWindowState` records. `HandleInteractBlock` validates reach + block kind, allocates a per-client windowId, opens against the right backing entity (chest entity slots aliased; furnace 3-slot snapshot synced back; crafting allocated fresh 9+1 grid). `HandleWindowClick` dispatches LMB/RMB/Shift via inline cursor-exchange logic (mirrors `Inventory.HandleLeftClickSlot` etc. on a (slot, cursor) pair). Crafting output slot recomputes via `CraftingRecipes.Match` after every input mutation; output click consumes one of each input. `HandleCloseWindow` commits cursor + crafting leftovers via `SpawnDropHook` (the same hook the host installs at `OpenToLan`).
    - **Friend client**: `_currentNetWindowId/Kind/Cell` state. `TryInteract` for Chest/Furnace/LitFurnace/CraftingTable sends `PlayerInteractBlock`. Inbound `OpenWindow` opens the right local panel + creates a stub tile entity at the cell so the existing `RenderChest` / `RenderFurnace` paths can read it. Inbound `TileEntityData` populates the local entity's slots. `HandleChestClick` and `HandleFurnaceClick` intercept in net mode: chest/furnace slots → `WindowClick`; player inventory slots → existing `InventoryClick`. `CloseChest` / `CloseFurnace` / `CloseCrafting` send `CloseWindow` (server commits the cursor and crafting leftovers).
    - **Limitations** (smaller polish, not blockers):
      - Crafting table input click forwarding for friends isn't yet wired (the friend can OPEN a crafting table panel; the server allocates the 9+1 grid; clicks within the panel still fall through to the host-style path because `HandleCraftingClick` doesn't yet have the net-mode branch — easy mirror of `HandleChestClick`).
      - Shift-click between window and player inventory uses two packet flavours (WindowClick for window slots, InventoryClick for player slots), which means the server can't smartly route a player-side shift-click into the open chest. Still cursor-driven moves work.
      - Furnace cook-while-watched-by-friend doesn't broadcast `TileEntityData` updates yet — friend sees the initial state on open but doesn't see the timer tick. Polish.
      - Multiple friends opening the same chest each see a snapshot at open-time; if A pulls an item, B's view is stale until B clicks. Per the spec's risk #2 this needs a "broadcast TileEntityData to every viewer of cell" pass.
