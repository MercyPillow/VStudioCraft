...
51. **Halloween Update block set** — Alpha 1.1.0 (Oct 30, 2010), in scope for our 1.1.2_01 target. Pumpkin already shipped (Tier 6 polish). Remaining:
    - **Nether-native mobs / finer terrain / MP sync** (V6+ swim-lane — V1 through V6 shipped).
        ...
        - **V7 (open) — Ghast**: flying Nether-native mob, sustained-line-of-sight aggro, fires a fireball projectile in a slow arc. Introduces `FireballProjectile` entity (reused by Blaze in V8). Roughly the size of a new mob class + one new projectile type.
        - **V8 (open) — Blaze**: flying Nether-fortress mob, 3-shot fireball volley with flame particle ring, hovers in place. Reuses `FireballProjectile` from V7. Drops 0..1 BlazeRod (new item, fuel for brewing — though brewing is out of scope until Beta).
