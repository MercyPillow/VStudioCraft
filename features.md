...
51. **Halloween Update block set** — Alpha 1.1.0 (Oct 30, 2010), in scope for our 1.1.2_01 target. Pumpkin already shipped (Tier 6 polish). Remaining:
    - **Nether-native mobs / finer terrain / MP sync** (V6+ swim-lane — V1 through V5 shipped). V5: full save/load round-trip for the Nether. v16 save format extends with a trailing block carrying nether chunks + nether-side tile entities (chest / furnace / sign / dispenser). `WorldSaveFormat.LoadWithPlayersAndNether` returns the nether World; `GameRenderer.LoadFromFile` stashes it in `_dormantWorld` so the next portal teleport finds the persisted terrain ready instead of regenerating fresh. The overworld is always the primary file content — saving while the player is in the Nether transparently writes the dormant overworld and reloads them at their last overworld portal-entry point. What remains for V6+: (a) Nether-native mobs (ghasts, blazes, zombie pigmen) — needs new entity classes + the projectile / aggro infrastructure each one carries; (b) finer terrain variety (ravines, fortress structures, wider lava seas); (c) multiplayer dimension sync (LAN coop currently shows the host's overworld to friends regardless of the host's actual dimension); (d) per-dimension player-position persistence (V5 round-trips the terrain but a save-and-quit while in the Nether always reloads in the overworld at the last portal-entry; the player's exact Nether coords are not preserved across reload).
