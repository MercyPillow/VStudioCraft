...
51. **Halloween Update block set** — Alpha 1.1.0 (Oct 30, 2010), in scope for our 1.1.2_01 target. Pumpkin already shipped (Tier 6 polish). Remaining:
    - **Nether-native mobs / finer terrain / MP sync** (V6+ swim-lane — V1 through V10 shipped).
        ...
        - **V11 (open) — Multiplayer dimension sync**: LAN coop currently shows the host's overworld to friends regardless of the host's actual dimension. Needs `Dimension` byte on player state packets, server-side per-dimension chunk views, and friend's `ChunkLoad` packets streamed from whichever dimension the host's perspective currently resolves to. The host's nether visit currently desyncs friend rendering — they keep seeing their cached overworld chunks while the host walks netherrack.
