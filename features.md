...
- Fluid simulation: source-driven outflow tick every 0.25s. Sources flow downward into Air at full reach, horizontally up to 4 cells (water — tighter than Alpha's 7 to keep undrained puddles modest) or 3 cells (lava). Per-cell 5-bit metadata (4-bit reach + falling marker) carries the flow state. Per-chunk `HasActiveFluid` flag self-deactivates steady-state chunks so a settled ocean costs nothing per tick; player edits re-engage the flag on the chunk + 4 neighbours. Light is recomputed only for chunks where lava propagated (water is light-transparent).
