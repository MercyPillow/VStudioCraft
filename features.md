...
32. **Ravines + dungeons (cobble rooms with spawner + chest)** — DONE. Dungeons via `World.GenerateDungeonsInChunk` (5×4×5 cobble rooms with mossy variants, mob spawner at floor centre, chest at random corner with weighted-loot table — 1-in-100 saddle / 3-in-100 bucket / common bread/wheat/sticks/gunpowder/string/iron). Ravines via `TerrainGenerator.GenerateRavines` using a feature-seed grid (96-block cells, ~1 in 8 cells seeded; each chunk queries the small set of cells whose ravine paths could reach into it and carves the slice that lands in its bounds — cross-chunk continuity is automatic because every chunk sees the same grid + seeded RNG). Centerline walks at unit steps; width tapers via sin(πt) so ends round off; carves stone/dirt/gravel/sand/grass/ores but spares bedrock + existing water.
