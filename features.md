...
51. **Halloween Update block set** — Alpha 1.1.0 (Oct 30, 2010), in scope for our 1.1.2_01 target. Pumpkin already shipped (Tier 6 polish). Remaining:
    - **Nether-native mobs / finer terrain / MP sync** (V6+ swim-lane — V1 through V11 shipped, swim-lane complete).
        - V5 (shipped): full save/load round-trip for the Nether. v16 save format extends with a trailing block carrying nether chunks + nether-side tile entities (chest / furnace / sign / dispenser). `WorldSaveFormat.LoadWithPlayersAndNether` returns the nether World; `GameRenderer.LoadFromFile` stashes it in `_dormantWorld` so the next portal teleport finds the persisted terrain ready instead of regenerating fresh. The overworld is always the primary file content — saving while the player is in the Nether transparently writes the dormant overworld.
        - V6 (shipped): three sub-parts — (1) per-dimension player-position persistence (save format v17 `DimensionState` trailing block: OverworldPos/Yaw/Pitch + NetherPos/Yaw/Pitch + CurrentDimension byte; LoadFromFile auto-swaps to Nether on reload if the player saved there); (2) finer Nether terrain (per-column ceiling height variance, surface lava pools, soul-sand cluster patches, gravel patches, wider lava lakes 3..6 radius, larger glowstone clusters 4..9 cells); (3) Zombie Pigman — first Nether-native mob with neutral aggro state machine (TakeDamage flips a 30s aggro timer), drops 0..2 CookedPorkchop + 1-in-4 GoldIngot, 1-in-150 per-column spawn density, wired into both the initial 5×5 ring and lazy-streamed chunks.
        - V7 (shipped) — Ghast: 4×4×4 hovering flying mob (no gravity, sin-bob + lateral drift), 10 HP, 36-block detect range, fires `FireballProjectile` every 3 s when LOS clear. New `FireballProjectile` class — straight-line, no-gravity, explodes (radius 2) on contact via the shared `Explode` helper. Spawns at 1-in-3000 per Nether column with a 4-block air-pocket clearance check; wired into both initial 5×5 ring and lazy-streamed chunks. Drops 0..2 Gunpowder. Renderer adds `_fireballs` list + `TickGhastFiring` + `TickFireballs` + `RenderFireballs` (orange glowing cube) + `DrawGhast` (white blob body + 9 dangling tentacles + eyes/mouth).
        - V8 (shipped) — Blaze: smaller flying mob (HalfWidth=0.4, h=2.0), 20 HP, 24-block detect, 3-shot volley state machine (3 shots × 0.30 s spacing, then 4 s rest) with ±~3° per-shot spread so a stationary blaze can't paint a perfect line on the player. Reuses `FireballProjectile` from V7. Spawns at 1-in-1500 per Nether column. Drops 0..1 GoldIngot (canonical BlazeRod is brewing-tied, brewing out of scope until Beta). Renderer adds `DrawBlaze` (orange-yellow core + three counter-rotating rings of dark rod cuboids, animated via per-mob `RodPhase` field).
        - V9 (shipped) — Nether ravines: terrain-gen extension in `NetherTerrainGenerator`. Each chunk inspects its own anchor + 8 horizontal neighbours; for each anchor that rolled a ravine (1-in-12), the gen pass walks the ravine's full path and carves cells that fall within the current chunk's bounds. The neighbour-walk pattern keeps ravine seams continuous across chunk boundaries (without it a ravine starting in one chunk and curving into another would cliff at the seam). Per ravine: 30..60 swept-capsule steps with yaw + pitch jitter, horizontal radius 2.5..4, vertical extent 6..10 (slot-shaped — taller than wide, distinguishes it from cave gen), starting at y=18..45 in mid-mass. Inserted between netherrack mass placement and lava lakes so a ravine cutting through y=31 lets the lava settle into the carved floor instead of generating floating slabs. Bedrock floor preserved.
        - V10 (shipped) — Nether fortress structures: 3×3-chunk fortress (48×48×6) anchored on the 8-chunk grid with 1-in-2 spawn chance per anchor, so a player wandering from spawn finds one within ~10 chunks on average. Reuses existing `Bricks` block (red brick) for walls + floor + ceiling — visually distinct against netherrack without needing a new tile slot for canonical nether brick. Layout: full 48×48 brick floor at y=72, 48-perimeter walls 4 tall (y=73..76) with 5-wide door cutouts on each cardinal side (lintel at top intact for a doorway feel), full 48×48 brick ceiling at y=77, hollow interior. Centre cell carries a `MobSpawner` block on a 1-cell brick pedestal — placeholder spawner-cage marker until functional spawners ship. Same 9-anchor neighbour-walk pattern as the ravine pass keeps multi-chunk seams continuous.
        - V11 (shipped) — Multiplayer dimension sync (minimum-viable scope): each `ServerClient` carries a `Dimension CurrentDim` field (defaults to Overworld). `ServerHub.SetHostDimension(Dimension)` is called from `GameRenderer.SwapDimension` so the LAN-host's in-process server learns about portal crossings. `BroadcastEntityUpdates` extends its viewer/target pair filter to skip pairs where dims differ — friends see an EntityDespawn for the host's avatar when the host crosses to Nether, and a fresh EntitySpawn (via the existing "in view, not yet tracked" branch) when the host returns. Friends are otherwise unaffected: their overworld chunks + entities + drops + tile entities all stay live; they just see the host disappear when the host walks through a portal. Per-dim chunk views for friends-following-host (full bidirectional dim travel for non-host players) is a future-polish item — friends currently can't trigger their own portal crossings, so the symmetric path doesn't fire today.

### Tier 9 — Infrastructure + completion

53. **Configurable key bindings — in-game capture UI** (V1 + V2 + V3 shipped). V2 shipped the rebinding mechanism: 8 gameplay keys (forward/back/left/right/jump/sneak/inventory/drop) routed through a `KeyBindings` static surface (`KeyBindings.MoveForward` etc.) instead of hardcoded `Keys.W`. Bindings load from `%APPDATA%\VStudioCraft\keybindings.cfg` on host construction and save on shutdown. V3 ships the in-game capture UI: new `ControlsMenu.cs` sub-screen of the Options menu (clicked via "KEY BINDINGS..." row), shows one row per binding with the current key's display name on the right; clicking a row arms capture mode (row paints in hot orange, "PRESS ANY KEY..." prompt) and the next non-Escape KeyDown commits the new binding + saves to disk. Esc cancels capture. "RESET DEFAULTS" button reverts to canonical Alpha defaults + saves. Capture KeyDown is intercepted ahead of all game shortcuts so a rebind to a hotbar key doesn't double-fire on the same press. F-keys (F3/F5/F8) and 1-9 hotbar selectors stay hardcoded.
