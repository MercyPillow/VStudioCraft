...
51. **Halloween Update block set** — Alpha 1.1.0 (Oct 30, 2010), in scope for our 1.1.2_01 target. Pumpkin already shipped (Tier 6 polish). Remaining:
    - **Nether dimension persistence + nether-native mobs** (V5 polish — V1 + V2 + V3 + V4 shipped). V4 ships the dimension itself: `Dimension` enum + `World.GenerateNether(seed)` factory + `NetherTerrainGenerator` (bedrock floor + cap, netherrack mass, lava-lake carving at y=31, glowstone clusters punched into the ceiling, soul-sand surface patches), `GameRenderer.SwapDimension(target)` (swaps `_world`, drops `_chunkMeshes`, restores per-dimension player position + camera yaw/pitch, lazy-creates the target World on first call), `BuildNetherStarterPortal()` for the first-time arrival, and `GenerateNetherChunk(cx, cz)` for synchronous Nether-side chunk streaming when the player wanders past the initial 5×5 ring. Dimension-aware sky / ambient / fog — flat dim red-orange sky, sky-light=0 (only block light reaches the cavern), fog clamped to ~36-block visibility with a red-orange tint. Hooks the V3 part 2 teleport trigger to actually call `SwapDimension`. What remains for V5: (a) save/load persistence for the Nether (current build regenerates the Nether fresh on world reload — overworld portal frames remember nothing about the nether's pre-built terrain), (b) Nether-native mobs (ghasts, blazes, zombie pigmen), (c) finer terrain variety (ravines, fortress structures, wider lava seas), (d) multiplayer dimension sync (LAN coop currently shows the host's overworld to friends regardless of the host's actual dimension).
