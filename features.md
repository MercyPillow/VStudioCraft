...
51. **Halloween Update block set** — Alpha 1.1.0 (Oct 30, 2010), in scope for our 1.1.2_01 target. Pumpkin already shipped (Tier 6 polish). Remaining:
    - **Nether portal teleportation + dimension** (V4 — V1 + V2 + V3 part 1 + V3 part 2 shipped). V1: block + 4×5 obsidian frame ignition + purple swirl texture. V2: frame-collapse on obsidian break (flood-fill from `World.SetBlock` dissolves the connected portal component when an adjacent obsidian is removed). V3 part 1: player-touch detection (`Player.IsInNetherPortal`) + per-frame `_inPortalSeconds` accumulator + indigo `RenderPortalTint` ramping to ~0.55 alpha at 3 s. V3 part 2: teleport TRIGGER fires when continuous contact crosses `PortalTeleportSeconds = 4 s` — plays a 0.45 s white-flash overlay (`RenderPortalTeleportFlash`), resets the accumulator, and locks a 1 s post-trigger cooldown so the trigger doesn't re-fire while the player is still in the portal. What remains for V4: actually swapping dimensions when the trigger fires — needs a separate chunk store / generation pass / round-trip return-portal placement. The trigger hook itself is the drop-in point for the eventual dimension swap.
