...
52. **Critical hits** — *missing.* No `crit` references anywhere. Alpha rule: hitting a mob while the attacker has `Velocity.Y < 0` (i.e. mid-air, falling) does +50% damage. Effort: ~10 LoC in the melee-hit damage path + a small particle burst.
53. **Spider wall climbing** — *missing.* Acknowledged in `HostileMobs.cs` ("Note: Alpha spiders climb walls, but vertical pathing is a much…"). Vertical pathing override on `Spider.Update`: when bumped horizontally against a solid block while the chase target is above, set Velocity.Y to a small positive climb rate. Effort: ~50 LoC.
54. **Double chest (large chest)** — *missing.* Explicit "single chest only (no double-chest pairing)" in `Block.cs`. Two adjacent placed chests should detect each other and open as a 54-slot pair. Mesh path stays single-cell; only the inventory-screen layout changes. Effort: ~250 LoC (pair-detection on place/break, 54-slot screen layout + click router).
55. **Compass needle** — *partial.* `DrawCompassDirectionMark` ships a single N/E/S/W letter overlay on top of the compass icon as a "complete-enough V1"; explicit `TODO: replace with a real rotated-needle quad` in `GameRenderer.cs`. Effort: ~80 LoC (rotated 2D quad path on the HUD layer).
56. **Clock item** — *missing.* Not in `BlockType` enum. World already has `TimeOfDay`. Render the item icon with a per-frame UV rotation indexed by `TimeOfDay`. Effort: ~120 LoC (item id + recipe + rotated icon render).
57. **Music disc audio playback** — *partial.* Jukebox tile entity, RMB-insert flow, and `AudioEngine.PlayMusic(buffer)` are wired; `buffer == 0` is treated as "no audio asset" and is a soft no-op. Comment says "real disc audio is a roadmap-deferred polish item until disc-asset wrangling is in scope." Effort: ~150 LoC + asset acquisition (PCM WAV/OGG embed + per-disc buffer table; the `PlayMusic` plumbing already exists).
58. **Runtime ice formation in cold biomes at night** — *missing.* Worldgen freezes water at chunk-create time (`FreezeWaterInSnowBiome`), but the runtime night-tick conversion of exposed water → ice in snow biomes isn't there. Effort: ~100 LoC (per-chunk snow-biome scan in the existing tick driver, gated on darkness).
59. **Map item** — *missing.* Not in `BlockType` enum. Empty-Map crafting + per-cell explored buffer + map-rendering on the held item. The most expensive item on the list — fills a 128×128 texture from a per-player explored set, persisted with the world. Effort: ~600 LoC + a save-format version bump.

---

### Tier 11 — Optional Features
60. **Smooth lighting / vertex AO** — Per-corner light sample at mesh time for ambient occlusion in cave/overhang corners.
61. **Animated water / lava textures** — moved to be last, original attempts edited the texture, instead of animating it. -> Frame-cycle a procedurally generated atlas-array layer so the surface shimmers / churns instead of staring back like wallpaper.
62. **Surface lava lakes** — Originally attempted under Tier 6 #33 alongside underground pools + cliff springs but removed: the flatness gate (sparse 8-point perimeter probe + windowed Y scan) plus stone-border ring still cost a ~300-FPS dip during chunk-stream-in because the cost lands on the chunk-job thread right when the renderer is also uploading meshes. Underground pools were kept because their pre-checks are O(1) cell reads. Revisiting needs either: (a) a precomputed per-chunk heightmap that this pass can sample without scanning Y at all, or (b) deferring lake placement to a separate post-gen pass that runs off the hot path. Visual payoff is small (occasional surface lava blob); only worth tackling once one of those infrastructure pieces lands for another reason.
