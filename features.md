...
53. **Configurable key bindings — in-game capture UI** (V3 — V1 + V2 shipped). V2 ships the rebinding mechanism: 8 gameplay keys (forward/back/left/right/jump/sneak/inventory/drop) routed through a `KeyBindings` static surface (`KeyBindings.MoveForward` etc.) instead of hardcoded `Keys.W`. Bindings load from `%APPDATA%\VStudioCraft\keybindings.cfg` on host construction and save on shutdown — a fresh install runs with canonical defaults; an existing file's `Name=KeyName` lines (e.g. `Forward=W`, `Sneak=ShiftKey`) override per-action. The host's KeyDown dispatch checks `KeyBindings.DropItem` / `Inventory` ahead of the hardcoded switch so a rebound key fires the action and the original Q/E are inert when reassigned. What remains for V3: an in-game capture UI in the options menu so the player can rebind without editing the config file by hand. F-keys (F3/F5/F8) and the 1-9 hotbar selectors stay hardcoded — they're not canonical user-rebindable in Alpha 1.1.2_01.
